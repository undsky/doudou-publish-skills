...
2. 静候 2.5 秒，让掘金编辑器原生自动保存机制生效；
3. 检测顶部状态文字（「保存成功」/「全部已保存」）以及 URL 中的草稿 ID（`https://juejin.cn/editor/drafts/<draft_id>`）；
4. 调用 `take_screenshot` 保存当前就绪页面截图作为存证（`juejin_draft_proof.png`）；
5. **保留页面现场**：存证完成后，**严禁调用 `close_page` 或关闭标签页**，保持当前页面打开供人工复核；
6. 输出结构化结果报告（文章标题、草稿 ID、分类、标签、摘要、封面状态等）。

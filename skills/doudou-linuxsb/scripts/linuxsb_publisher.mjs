 ...
 * 【核心铁律】本流程仅负责自动填入标题、Markdown正文及实时预览校验，严禁自动触发保存提交！
 * 打开 URL (fid=4) 默认即为技术交流版块，无需重复判断与选择版块。
 * @param {string} markdownFilePath 
 * @param {{ preferCdn?: boolean }} options 
...
export function buildBrowserPublishScript(markdownFilePath, options = {}) {
  ...
  // 4. 拟真注入 Markdown 正文并同步 NB-Editor
  ...
  log('Linux.sb 文章内容填充完毕，已就绪，保持编辑态供用户人工审查与手动保存。');
...
if (process.argv[1] && path.resolve(process.argv[1]) === path.resolve(new URL(import.meta.url).pathname.replace(/^\/([A-Z]:)/, '$1'))) {
  ...
  if (!filePath) {
    console.log('Usage: node linuxsb_publisher.mjs <MarkdownFilePath>');
    process.exit(1);
  }
  const scriptCode = buildBrowserPublishScript(filePath);

---
name: doudou-aliyun
description: 通过 chrome-devtools-mcp 实现将本地 Markdown 文章自动发布到阿里云开发者社区草稿箱（https://developer.aliyun.com/article/new）。支持真实人工行为模拟、防风控时延与事件派发、智能封面提取（兼容 doudou-markdown 产物同名目录与 cdn_manifest.json）、CDN 正文自动替换以及草稿保存状态验证。
---
...
2. **防风控与真实人机行为模拟 (Anti-Bot & Human Simulation)**：
   ...
   - **真实事件完整性**：对于表单输入，依次派发 `focus`、`keydown`、`input`、`keyup`、`change`、`blur`，并同步更新 React Field 与 Component State。
...
3. **资产自动解析优先级**（参考 `doudou-markdown-skill` 规约）：
   ...
   - **封面图**：
     1. 优先读取同名目录下 `cdn_manifest.json` 中 `type: "cover"` 的 CDN 链接（优先 2.35:1 / 16:9 宽屏封面）；
...
3. 设置标题值并同步触发 React Field 与 DOM 事件：
```javascript
...
1. 在浏览器端将图片构造为 `File` 对象；
2. 调用表单组件的 `instance.uploadCoverImage(file, 'coverImage')` 方法，利用阿里云官方签名通道上传至 OSS 并自动绑定为封面；
3. 监听 `instance.state.fileList` 确认封面上传成功并展示缩略图：
```javascript
async function uploadCover(coverInfo, formInstance) {
  let file;
  if (coverInfo.type === 'cdn' && coverInfo.url) {
    const resp = await fetch(coverInfo.url);
    const blob = await resp.blob();
    file = new File([blob], 'cover.jpg', { type: blob.type || 'image/jpeg' });
  } else if (coverInfo.base64) {
    const byteCharacters = atob(coverInfo.base64);
    const byteNumbers = new Array(byteCharacters.length);
    for (let i = 0; i < byteCharacters.length; i++) {
      byteNumbers[i] = byteCharacters.charCodeAt(i);
    }
    const byteArray = new Uint8Array(byteNumbers);
    const blob = new Blob([byteArray], { type: coverInfo.mimeType || 'image/png' });
    file = new File([blob], 'cover.png', { type: blob.type });
  }
  
  if (!file) return { success: false, reason: 'no valid cover file' };

  return new Promise((resolve) => {
    const origSetState = formInstance.setState.bind(formInstance);
    let done = false;
    formInstance.setState = function(partialState, callback) {
      origSetState(partialState, () => {
        if (callback) callback();
        if (partialState && partialState.fileList && partialState.fileList.length > 0) {
          done = true;
          resolve({ success: true, fileList: partialState.fileList });
        }
      });
    };

    formInstance.uploadCoverImage(file, 'coverImage');

    setTimeout(() => {
      if (!done) {
        resolve({ success: (formInstance.state.fileList?.length > 0), fileList: formInstance.state.fileList });
      }
    }, 12000);
  });
}
...
2. 调用 `take_screenshot` 保存当前视口截图作为存证。

...
1. **发布入口与多模态创作**：
   - 微信公众平台入口：`https://mp.weixin.qq.com`（草稿箱管理界面）
   - 技能原生支持**「图文文章 (Article)」**与**「图文贴图 (Sticker)」**双模态创作发布。
2. **模态判定与串行执行原则**：
   - **默认全模态**：用户未明确指定模态时，默认发布全部可用模态（图文文章 + 小绿书贴图全发）。严禁自行缩减模态或中途反问用户。
   - **显式指定**：用户明确指定（如「只发文章」「仅发贴图」）时，严格按指定集合执行。
   - **资产缺失处理**：若缺少对应资产（无可用排版 HTML 跳过文章，无贴图卡片集跳过贴图），自动跳过并在报告与回执中登记原因，绝不中断其余模态。
   - **执行顺序与隔离**：恒为 `article` → `sticker`（先长文后贴图）；各模态必须回到草稿箱重新点击「新的创作」选择对应入口，隔离状态；两个模态保存草稿间隔至少 3 秒；单个模态失败仅记录并继续后续模态。
3. **模态可用性判定与确定性计划**：
   ...
   > 封面图缺失不构成跳过文章模态理由：按既有异常规约降级跳过封面注入，正文草稿照常保存并在报告中标记待手动绑定。

   `parseAllAssets()` 已内置模态编排，直接读取 `meta.publishPlan`：
   ```bash
   node scripts/parser.mjs <Markdown文件绝对路径>          # 默认全模态
   node scripts/parser.mjs <Markdown文件绝对路径> "贴图"    # 仅指定模态
   ```
4. **资产规范与限制**：
   - **文章正文 HTML**：必须优先读取同名目录下由 `gzh-design` 生成的**纯排版正文 HTML**（`_排版_主题(ID).html`，绝对不要使用带复制工具栏的 `_预览.html`）。通过剪贴板 `paste` 事件原生注入并由官方 DOMParser 解析；
   - **文章封面图**：严格优先选用 **2.35:1 宽屏主封面**，且优先选用 `_thumb` 缩略图（如 `cover-2.35x1_thumb.png`，次选 `cover-2.35x1.png` 或 `cover-16x9.png`）；
   - **贴图卡片集**：读取同名目录下 `xhs_images/images/` 的所有卡片图片（排除 `*_yuantu.png` 原图），按序号升序批量上传；
   - **标题与摘要**：文章标题 `<= 64 字`（坚决不填作者、不填摘要）；贴图标题 `<= 20 字`，贴图描述 `100~300 字` + `#话题标签`。
5. **安全隔离（绝对底线）**：
   - **绝对严禁触碰任何形式的公开发布按钮**：绝不点击「发表」或「群发」按钮；
   - **依托平台原生自动保存**：公众平台在标题、正文及封面配置后具备原生防抖自动存草稿机制；脚本在注入完成后平滑滚动视口模拟人工排版审阅，轮询完成断言直至通过，无需强行寻找保存按钮。
6. **完成判定必须客观可断言（严禁以「已等待」代替「已完成」）**：
   - 「静候自动保存生效」是等待动作，不是验收条件。必须以完成断言求值为 `true` 才允许判定完成。
   - 断言未通过即为未完成：登记 `timeout` 并保留页面，**严禁登记为 `success`**。
7. **收尾必须落盘回执（父级编排的唯一完成信号）**：
   - 无论成功、失败、缺登录、超时还是跳过，收尾都必须调用 `scripts/receipt.mjs write` 写入回执。
   - 父级 `doudou-UGC-skill` 不解析自然语言汇报，只读回执文件；缺回执会让父级永久阻塞后续平台。
8. **防风控与真实人机行为模拟 (Anti-Bot & Human Simulation)**：
   - **随机时延抖动**：输入前 300~600ms、步骤间 600~1500ms、点击前 400~700ms，严禁毫秒级并发；
   - **真实事件完整性**：对于表单输入，依次派发 `focus`、`keydown`、`input`、`keyup`、`change`、`blur`，并同步底层隐藏元素与 ProseMirror 编辑器；
   - **平滑视口滚动**：模拟人类自上而下的视觉审查，分步平滑滚动页面触发可见性检测；
   - **拟真悬停与交互**：交互前先将视口滚动到元素可见区域，派发 `mouseover`/`mouseenter` 悬停 400~600ms 后再交互。
9. **发布完成后保留页面（严禁自动关闭）**：
   - 图文与贴图草稿保存及存证截图完成后，**严禁调用 `close_page` 或以任何方式关闭编辑器页面与公众平台后台页面**，必须原样保留页面现场，供人工最终核验。
...
统一存至同名文章目录下的 `publishes/screenshots/` 目录（格式 `<platformSlug>_<mode>.png`）：
- 图文文章：`publishes/screenshots/weixin_article.png`
- 小绿书贴图：`publishes/screenshots/weixin_sticker.png`

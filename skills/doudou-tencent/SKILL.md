...
运行辅助解析脚本或内置逻辑提取元数据：
```bash
node /Users/jyx/project/doudou-tencent-skill/scripts/parser.mjs <Markdown文件绝对路径>
```
输出包含：
...
```bash
node /Users/jyx/project/doudou-tencent-skill/scripts/tencent_publisher.mjs <Markdown文件路径>
```

2. **在 Agent 中配合 `chrome-devtools-mcp` 调用**：
```javascript
import { buildBrowserPublishScript } from '/Users/jyx/project/doudou-tencent-skill/scripts/tencent_publisher.mjs';

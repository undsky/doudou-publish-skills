 ...
 * 涵盖：
 * 1. 发布文章：TipTap/ProseMirror 纯排版 HTML 正文双向同步、文章头图与封面设置真实上传、确定性弹窗点击、防风控人机交互与草稿暂存
 * 2. 发布图文：真实 5 张卡片本地文件批量上传、20 字标题与 1000 字话题描述填充、防风控人机交互与草稿暂存
 */
...
export function buildArticleBrowserScript(meta) {
  ...
  if (titleInput) {
    titleInput.focus();
    await sleep(250);
    setInputValue(titleInput, meta.title);
    titleInput.blur();
  }
  await sleep(350);
  ...
  if (summaryInput) {
    summaryInput.focus();
    await sleep(250);
    setInputValue(summaryInput, meta.summary);
    summaryInput.blur();
  }
  await sleep(350);
  ...
  if (meta.coverBase64) {
    try {
      ...
      const coverBtn = document.querySelector('.addIcon-WtgoEN') || document.querySelector('.mycard-info-uBuPL9') || document.querySelector('.content-upload-ksKds3');
      if (coverBtn) {
        coverBtn.click();
        await sleep(300);
        document.createElement = origCreate;
        ...
        if (coverInput) {
          const file = makeFile(meta.coverBase64, 'cover-main-2.35x1.png');
          const dt = new DataTransfer();
          dt.items.add(file);
      ...
      console.warn('[doudou-douyin] 封面上传异常:', e);
    }
  }
...
export function buildImagePostEditorScript(meta) {
  ...
  console.log('[doudou-douyin] 开始填写图文作品信息并保存草稿...');
  ...
  if (descEl) {
    ...
    let docExecOk = false;
    try {
      docExecOk = document.execCommand('insertText', false, meta.description);
    } catch (e) {
    ...
    if (!docExecOk || descEl.innerText.trim().length < 10) {
      ...
      while (curr) {
        if (curr.memoizedProps?.editor && typeof curr.memoizedProps?.editor?.setContent === 'function') {
          curr.memoizedProps.editor.setContent(meta.description);
          break;
        }
        curr = curr.return;
      }

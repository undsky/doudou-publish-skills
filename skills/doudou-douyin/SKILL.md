---
name: doudou-douyin
description: "通过 chrome-devtools-mcp 实现抖音发布文章、图文到草稿箱功能。支持根据用户指定的 Markdown 文章及其同名资产目录，真实上传高清封面（>=500px校验）、纯排版富文本正文（TipTap状态双向同步）、全套真实小红书/抖音信息图卡片（01~05高清PNG）、异步CDN上传就绪轮询、React Fiber话题绑定，全流程模拟真实人类行为防风控并存证。"
---
...
2. **异步上传等待机制（核心）**：图文卡片上传派发后，平台后台需要 10~20 秒进行 CDN 转存与切片，**必须异步轮询等待**直到出现「已添加N张图片」（或「取消上传」消失），确认手机预览区图片加载完毕后方可继续填写与暂存，严禁提前暂存导致图片丢失。
3. **随机微延迟**：在表单聚焦、输入、点击之间插入 200ms ~ 600ms 随机延迟（`sleep(ms + Math.random() * 200)`）。
4. **原生事件与状态双向同步**：文本输入必须派发 `input` 与 `change` 事件；富文本调用 TipTap `setContent`、`onUpdate` 与 `emit` 同步；话题标签通过 React Fiber 的 `setItem` 直接同步 `long_article_topic`。
5. **真实鼠标交互**：点击操作前先将元素 `scrollIntoView({ behavior: 'smooth' })`，派发 `mouseover`、`mouseenter` 再执行 `click`。
6. **确定性草稿保存与存证**：全流程终点统一点击「**暂存离开**」（严禁误触直接「发布」），并在完成暂存后截取存证图片（`douyin_draft_proof.png` / `douyin_image_draft_proof.png`）。
...
- [scripts/parser.mjs](scripts/parser.mjs)：解析 Markdown、提取长文/图文标题（防超长截断）、摘要、话题、排版 HTML、高清封面及图文卡片集。
- [scripts/douyin_publisher.mjs](scripts/douyin_publisher.mjs)：文章与图文发布浏览器注入脚本生成器（含放弃旧草稿、CDN上传等待轮询、React Fiber 状态双向同步与暂存草稿）。
